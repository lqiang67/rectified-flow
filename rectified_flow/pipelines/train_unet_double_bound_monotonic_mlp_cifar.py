...
def parse_args():
    ...
    parser.add_argument(
        "--use_monotonics",
        action="store_true",
        help="Whether to use monotonic MLPs in the boundary mlp.",
    )
...
def main(args):
    ...
    model = DoubleBoundMonotonicMLP(UNet_config, data_mean=data_mean, use_monotonics=args.use_monotonics)
    print("Whether using monotonic MLPs in the boundary mlp: ", args.use_monotonics)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for i, model in enumerate(models):
                if isinstance(accelerator.unwrap_model(model), DoubleBoundMonotonicMLP):
                    unwrap_model = accelerator.unwrap_model(model)
                    # unwrap_model.save_pretrained(output_dir, filename="unet")
                    state_dict_cpu = {k: v.cpu() for k, v in unwrap_model.state_dict().items()}
    ...
    if accelerator.is_main_process:
        tracker_name = "boundary_model_exps"
