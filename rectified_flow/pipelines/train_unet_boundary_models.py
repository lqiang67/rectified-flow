...
from rectified_flow.models.unet_bound import SongUNet, SongUNetConfig
...
def parse_args():
    ...
    parser.add_argument(
        "--bound_type",
        type=str,
        choices=["none", "single", "double", "single_sin", "double_sin"], # 限制可选值
        default="none",
        help="Boundary condition type: 'none', 'single', or 'double'."
    )
    def parse_bound_order(value):
        if value == "sin": return value
        try: return float(value)
        except ValueError: raise ValueError(f"Invalid bound_order: {value}. Must be a float or 'sin'.")
    parser.add_argument(
        "--bound_order",
        type=parse_bound_order,
        default="1",
        help="Order of the boundary condition: must be a float or 'sin'."
    )
...
def main(args):
    ...
    cifar_images = torch.stack([train_dataset[i][0] for i in range(len(train_dataset))])
    cifar_images = cifar_images.to(accelerator.device, dtype=weight_dtype)
    data_mean = cifar_images.mean(dim=[0], keepdim=True)
    del cifar_images
    ...
    UNet_config = SongUNetConfig(
        ...
        bound_order=args.bound_order,
    )
    model = SongUNet(UNet_config, data_mean=data_mean)
